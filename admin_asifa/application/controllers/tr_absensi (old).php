...
class tr_riwayat extends crud_periodik_controller {
	...
	public $table = 'tb_akd_tr_nilai_riwayat';
	...
	protected function renderTable($kolom, $table, $primary_key)
	{
		...
		if(is_array($jenis_riwayat)) foreach($jenis_riwayat as $key => $val)
		{
			...
			$header[] = "SUM(IF(a.kode_jenis_riwayat = '".$val->kode_jenis."', a.jumlah, 0)) as jenis_$key";
		}
		...
		LEFT JOIN
			tb_akd_tr_nilai_riwayat a ON a.no_induk=ak.no_induk AND a.kode_thn_ajaran=ak.kode_thn_ajaran AND a.kode_jenjang=ak.kode_jenjang
		WHERE
	...
	public function save()
	{
		...
		$this->load->model('mod_riwayat');
		$res = $this->mod_riwayat->saveSingleAbsensi($_POST['no_induk'], $_POST['data']);
		die( $res ? 'ok' : 'not ok');
	}
	...
	public function save_batch()
	{
		if ($this->input->server('REQUEST_METHOD') != 'POST') return false;
		$this->load->model('mod_riwayat');
		$res = $this->mod_riwayat->saveBatchAbsensi($_POST['data']);
		die( $res ? 'ok' : 'not ok');
