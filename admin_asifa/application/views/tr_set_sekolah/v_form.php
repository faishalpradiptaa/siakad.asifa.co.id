...
<div class="modal-header">
	...
	<h4 class="modal-title" >Pembagian Siswa ke Sekolah </h4>
</div>
		...
		<div class="row">
			...
			<div class="col-md-6">
				<div class="form-group">
					<label>Sekolah yang akan diisi</label>
					<div class="row">
						<div class="col-md-7">
							<select class="form-control" name="out_sekolah" validate="required">
								<option value="">-- Pilih Sekolah --</option>
								<?php 
									if(is_array($sekolah)) foreach ($sekolah as $row) echo '<option value="'.$row->kode_sekolah.'">'.$row->nama_sekolah.'</option>'; 
								?>
								<option value="-reset-">-- Reset Sekolah --</option>
							</select>
						</div>
						<div class="col-md-5">
							<select class="form-control" name="out_jurusan" >
								<option value="">-- Pilih Jurusan --</option>
								<?php 
									if(is_array($jurusan)) foreach ($jurusan as $row) echo '<option value="'.$row->kode_jurusan.'">'.$row->nama_jurusan.'</option>'; 
								?>
							</select>
						</div>
					</div>
				...
				<div class="form-group">
					...
					<select class="form-control" name="in_source">
						...
						<option value="sekolah">Berdasrkan sekolah di tahun ajaran sebelumnya</option>
						...
						<option value="non-sekolah">Yang belum punya sekolah</option>
					</select>
				</div>
				
				<div class="form-group hide" in-group="sekolah">
					<label>Pilih Tahun Ajaran sebelumnya</label>
					<div class="row">
						<div class="col-md-8">
				...
				<div class="form-group hide" in-group="sekolah">
					...
					<div class="row">
						<div class="col-md-7">
							<select class="form-control" name="in_sekolah">
								<option value="">-- Pilih Sekolah --</option>
								<?php 
									if(is_array($sekolah)) foreach ($sekolah as $row) echo '<option value="'.$row->kode_sekolah.'">'.$row->nama_sekolah.'</option>'; 
								?>
						...
						<div class="col-md-5">
							<select class="form-control" name="in_jurusan" >
								<option value="">-- Pilih Jurusan --</option>
								<?php 
									if(is_array($jurusan)) foreach ($jurusan as $row) echo '<option value="'.$row->kode_jurusan.'">'.$row->nama_jurusan.'</option>'; 
								?>
...
<script>
	$('[name="in_source"]').change(function(){
		if($(this).val() == 'non-sekolah') getSiswa('by_non_sekolah');
	...
	var in_sekolah = $('[name="in_sekolah"]');
	var in_jurusan = $('[name="in_jurusan"]');
	...
	$('[name="in_sekolah"],[name="in_jurusan"]').change(function(){
		getSiswa('by_sekolah');
	})
	...
	{
		var my_data = {};
		if(tipe == 'by_sekolah') my_data = {kode_thn_ajaran : in_thn_ajaran.val(), sekolah : in_sekolah.val(), jurusan : in_jurusan.val()};
		else if(tipe == 'by_non_sekolah') my_data = {non_sekolah : true};
		else if(tipe == 'by_kelas') my_data = {kelas : in_kelas.val()};
