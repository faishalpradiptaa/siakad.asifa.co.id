...
class mod_template_rapor extends MY_Model
{
	...
	private $thn_ajaran = false;
	private $template_dir = 'form';
	
	public function getDetail($kode, $thn_ajaran)
	{
		$this->thn_ajaran = $thn_ajaran;
		return $this->db
			->where('kode_template', $kode)
			->where('kode_thn_ajaran', $this->thn_ajaran)
	...
	public function getDetailPoint($kode_template, $id)
	{
		$where = array(
			...
			'kode_thn_ajaran' => $this->thn_ajaran,
	...
	public function getTreePoint($kode_template, $is_data=false)
	{
		$where = array(
			...
			'pr.kode_thn_ajaran' => $this->thn_ajaran,
	...
	public function getTemplateView($kode_template, $thn_ajaran, $siswa, $view = 'view')
	{
		$html = '';
		$this->thn_ajaran = $thn_ajaran;
	...
	{
		$exception = array('grafik_by_aspek');
		$html = '';
		$html .= '<div class="row rapor-point"><div class="col-md-12">';
		$html .= in_array($point->tipe_point, $exception) ? '' : '<h4>'.$point->nama_point.'</h4>';
		$html .= '<div class="margin-l-15">';
	...
	private function generatPointMapel($point, $siswa)
	{
		...
		WHERE
			...
			AND am.kode_thn_ajaran = '".$this->thn_ajaran."'
		...
		WHERE
			n.kode_thn_ajaran = '".$this->thn_ajaran."'
	...
	private function generatPointRiwayat($point, $siswa)
	{
		...
		$data = $this->db
			...
			->join('tb_akd_tr_riwayat a', 'ja.kode_jenis=a.kode_jenis_riwayat AND no_induk="'.$siswa->no_induk.'" AND kode_thn_ajaran="'.$this->thn_ajaran.'" AND kode_jenjang="'.JENJANG.'" ', 'left')
	...
	private function generatPointDeskripsi($point, $siswa)
	{
		...
		return '<div class="textarea">'.$data->deskripsi.'</div>';
	}
